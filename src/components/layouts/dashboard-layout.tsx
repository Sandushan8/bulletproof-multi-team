...
import { SwitchTeam } from '@/components/ui/switch-team/switch-team';
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const currentTeam = sessionStorage.getItem('currentTeamId');
    const selectedTeams = JSON.parse(
      sessionStorage.getItem('selectedTeamIds') || '[]',
    );
    const allTeams = teamsQuery.data?.data;
    if (selectedTeams.length > 0) {
      setCurrentTeam(
        selectedTeams.find((team: Team) => team.id === currentTeam),
      );
    } else if (allTeams && allTeams.length > 0) {
      setCurrentTeam(allTeams.find((team) => team.id === currentTeam));
    }
  }, [isOpenModal, teamsQuery.data?.data]);
    ...
    <div className="flex min-h-screen w-full flex-col bg-muted/40">
      ...
      <div className="flex flex-col sm:gap-4 sm:py-4 sm:pl-60">
        <header className="sticky top-0 z-30 flex h-14 items-center justify-between gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:justify-end sm:border-0 sm:bg-transparent sm:px-6">
          ...
          <DropdownMenu>
            Current Team:{' '}
            {currentTeam?.name ? currentTeam.name : 'No team selected'}
